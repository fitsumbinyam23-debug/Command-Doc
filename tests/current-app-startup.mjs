...
const assert = (condition, message) => {
  if (!condition) throw new Error(`Current startup test failed: ${message}`);
};
...
const server = http.createServer(async (request, response) => {
  ...
  if (!target.startsWith(client)) { response.writeHead(403).end(); return; }
  try {
...
try {
  ...
  assert(rootResponse.ok, "root URL loads from dist/client");
  ...
  console.log(JSON.stringify({ suite: "current startup", root: `${base}/`, active_assets: assets.length, runtime_json: data.length, passed: 3 + assets.length + data.length }));
} finally {
